...
const { login, app } = config.url;

const defaultProps = {};

class ReactComponentBase<P, S> extends React.PureComponent<P, S> {
    ...
    stateChange = (key, value, fnCb = () => { }) => {
        if (typeof key === 'string') {
            this.setState(
                {
                    // hack 手段，避免单独使用[key]: value,
                    // ts 检测报错 T2345 Argument of type '{ [x: string]: any; }' is not assignable to parameter of type
                    ...{},
                    [key]: value
                },
                fnCb
...
ReactComponentBase.defaultProps = defaultProps;
